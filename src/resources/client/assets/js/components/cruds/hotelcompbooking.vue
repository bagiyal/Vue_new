            ...
            <tab-content v-for="(loc,index) in location" :id="'hotel' + index" :key="'hotel' + index" :title="loc.name" icon="fa fa-check">
                ...
                <!--            Hotel mail Modal-->
                    <div class="container"  v-if="show_modal5">
                        <div class="modal show in" id="myModal5" style="display: block;">
                            <div class="modal-dialog" style="max-width: 750px !important;">
                                <div class="modal-content">

                                        <div class="modal-body" style="max-height: 600px !important;overflow: auto;">
                                        <label for="message" style="color: gray;float: left">Choose & Send Mail To Hotel</label>
                                    <button type="button" style="font-size: 26px;" class="close" data-dismiss="modal" @click="show_modal5=false">&times;</button><br>
                                    <div class="container-fluid" style="text-align: left">
                                        <h2 style="text-align: left"></h2>
                                        <hr>

                                        <div class="form-group container-fluid" v-for="(loc,index) in location">

                                            <div  class="col-md-1" style="margin-top: 20px;">
                                                <label class="container_input">
                                                    <input  type="checkbox" checked="checked" :id="hotelselect[loc.id]['title']" >
                                                    <span class="checkmark" style="border:1px solid; width: 23px !important;height:23px !important;"></span>
                                                </label></div>

                                            <!--                                            <div class="col-md-1">-->
<!--                                               -->
<!--                                                <label for="message" style="color: gray;font-size: 15px;"></label><br>-->
<!--                                                <div class="checkbox">-->
<!--                                                    <label><input type="checkbox" checked="checked" :id="hotelselect[loc.id]['title']"  class="checkbox2" >-->
<!--                                                        <span class="box"></span>-->
<!--                                                    </label>-->
<!--                                                </div>-->
<!--                                            </div>-->
                                            <div class="col-md-7">
                                                <label for="message" style="color: gray;font-size: 15px;">Hotel Name</label><br>
                                                {{hotelselect[loc.id]['title']}}
                                            </div>
                                            <div class="col-md-4">
                                                <label for="message" style="color: gray;font-size: 15px;">Email</label><br>
                                                <input type="text" v-model="local_tour[index]['hotel_email'] "   class="form-control">
                                            </div>
                                        </div>

                                    </div>

                                    <button type="button" class="btn btn-success" style="background: rgb(34, 80, 125) none repeat scroll 0% 0%;float: right;margin-top: 20px;font-size: 15px;" @click="send_mail_to_hotel(hotelselect[loc.id])" >Send Mail</button>
                                </div>
                            </div>
                        </div>
                    </div>
                                        ...
                                        <div class="card">
                                            <div class="fluid-container pl-2">
                                                ...
                                                <div class="row pr-3 pl-2">
                                                    <div class="col-12 text-left text-muted"><span><img src="./dashboard_resources/hotel name_icon.png" class="hw-3"></span>Hotel Name
<!--                                                  <i style="float:right" @click="callhotel()" aria-hidden="true" class="fa fa-refresh f-2"></i>-->
                                                    ...
                                                    <div class="col-12"  >
                                                        ...
                                                        <v-select v-if="!newhotelflag"  :clearable="false" v-model="hotelselect[loc.id]"
                                                                  name="addhotel"
                                                                  label="title"
                                                        ...
                                                        />
                                                        <v-select v-else :clearable="false" v-model="hotelselect[loc.id]"
                                                                   name="addhotel"
                                                                   label="title"
                                                                   @input="addtohotel(index,loc.id)"
                                                                   :value="non"
                                                                   :options=newhoteloption[loc.id]
                                                        />
...
//    alert("me empty2 ");
                ...
                if(this.hotel_image_link!='' || this.custom_hotel_image!='' ){
                    ...
                    axios.post('/api/v1/accommodations', params)
                        .then(response => {
                            ...
                            if(Object.keys(this.newhoteloption).length>0){
                                ...
                                this.newhoteloption=_.cloneDeep(this.hoteloptions);
                                this.newhoteloption[locid].push(response.data.data)
                                // Object.assign( this.newhoteloption, {[locid]: [response.data.data]})
                                console.log(this.newhoteloption);
            ...
            send_mail_to_hotel(data){
                var id='';
                var email='';
                var hotel='';
                var checked_hotel=[];
                var counter=0;
                for(var g=0;g<this.location.length;g++){
                    id=this.location[g]['id'];
                    hotel = document.getElementById(this.hotelselect[id]['title']).checked;
                    if(hotel==true){
                        checked_hotel.push({id:id,email:this.location[g]['hotel_email'],title:this.hotelselect[id]['title']})
                    }
                }
                if(checked_hotel.length>0){
                    for(var j=0;j<checked_hotel.length;j++){
                        if(checked_hotel[j]['email']!='' || checked_hotel[j]['email'].length>0){
                            counter=0;

                        }
                        else{

                            alert('Please add '+checked_hotel[j]['title']+" email id");
                            counter=1;
                            return false;
                        }
                    }
                ...
                if(counter==0){
                    ...
                    let params=new FormData();
                    params.set('book_id',this.booking_id);
                    params.set('data',JSON.stringify(checked_hotel));
                    this.$emit('save_booking');
                    axios.post('/api/webservices/send_hotel_mail' , params)
                        .then(response =>{
        ...
        props: ['location','hotels','tour_location','day','modal','booking_id']
