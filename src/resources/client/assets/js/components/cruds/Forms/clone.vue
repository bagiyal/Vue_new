<template>
    <section class="content-wrapper" style="min-height: 960px;margin-left: 5% !important;margin-top:3rem;">

        <section class="content-header" style="display:flex;">
            <task-maker :mode="'full'"></task-maker>
            <h1>Form Clone <i title="Form Preview" @click="preview=true" aria-hidden="true" class="fa fa-eye  "></i></h1>
                                            ...
                                            <div id="exTab2" class="col-md-12">
                                                <ul class="nav nav-tabs">
                                                    ...
                                                    <li v-for="(data, ind) in item.languages" :class="activeTab === ind ? 'active' : ''">
                                                        <a @click="selectLanguage(data)" style="font-size: 18px; color: black;">{{ data.name }}</a>
                                                      </li>
...
export default {
    ...
    computed: {
        ...mapGetters('FormsSingle', ['item', 'loading','clone_data','fields_arr'])
    },
    created() {
        ...
        if(typeof (this.$route.params.clone_data)!=="undefined"){
            ...
            // var type = Object.entries(this.$route.params.clone_data);

          this.item.form_html=_.cloneDeep(JSON.parse(this.$route.params.clone_data.form_html))
            this.item.banner=_.cloneDeep(this.$route.params.clone_data.banner)
            this.item.footer=_.cloneDeep(JSON.parse(this.$route.params.clone_data.footer))
            this.item.header=_.cloneDeep(JSON.parse(this.$route.params.clone_data.header))
            this.item.languages=_.cloneDeep(JSON.parse(this.$route.params.clone_data.languages))
    ...
    methods: {
        ...mapActions('FormsSingle', ['storeData', 'resetState', 'setForm_name', 'setAgency_id', 'setForm_html','setclone_data','setfields_arr','setAgent_id']),
        updateForm_name(e) {
        ...
        filterLang(i){
            ...
            this.item.form_html.forEach(itm =>{
                if(itm.ques !== undefined || itm.questions !== undefined){
                    ...
                    if(itm.type=='Check Box' || itm.type=='Radio Button'){
...
console.log(type);
    ...
    console.log( this.actualfield)
        ...
        nextHeader(){
         ...
         nextTab() {
               console.log("next work",this.numCount)
                ...
                const Count=this.numCount;
                console.log("lancode",this.item.languages)
                const sublis = this.item.languages[Count].code;
