    ...
    <section class="content-wrapper" style="margin-left: 5% !important;min-height: 960px;margin-top:3rem;">
        <section class="content-header">
            ...
            <div style="display:flex;">
                <span style="font-size: 25px">Form Create <i title="Form Preview" @click="preview=true" aria-hidden="true" class="fa fa-eye  "></i></span>
...
<!--                                                                                <span style="font-size: 16px;color:black" v-if="activeTab>0"><b>Option are:-</b></span><br>-->
            ...
            <transition name="modal">
                <div class="modal-mask">
                    <div class="modal-wrapper backGround">
                        <div class="modal-dialog form-body " >
                            <div class="modal-content" style="height: 600px;overflow-x: hidden;overflow-y: scroll">
                                ...
                                <div class="modal-body" style="">
                                    <FormComponent :hub_data_list="[]" :languages="item.languages" :banner="item.banner" mode="preview" :form_data="item" form_filler="handler" tabsVisibility="{'docs':false,'passport':false}" states="[]" />
            ...
            <div class="row">
                <div class="col-xs-12">
                    ...
                    <div  class="container-fluid"  :style="{ 'background-image': 'url('+background_url+')'}" style="background-repeat: no-repeat;background-size: 100% 100%;padding: 20px;background-size: cover;">
                        <i style="float: right;color: grey;font-size: 18px;" title="Delete Banner" alt="Delete Banner" v-if="banner_link!==''" @click="deletefetureimage" class="fa fa-trash"> </i>
                            ...
                            <div class="row mt-3">
                                ...
                                <div class="col-md-12 col-sm-12 text-center f-1-6" v-if="banner_link==''">
                                    ...
                                    <label class="file_hidden text-white">
                                        ...
                                        <input type="file" size="20" style="display:none !important;" @change="updatemeta_banner($event,'form_banner',index)" accept="image/x-png,image/jpeg">
                                    </label>
                                </div>
...
<!--                        <textarea class="field_class" v-model="item.header" cols="100" rows="3" style="width: 100%;font-size: 16px;"></textarea>-->
                        ...
                        >
                            ...
                            <div class="col-md-12 bg-white content_div " v-if="tag.key==='dob'" >
                                                    ...
                                                    <!--                                <label for="passport_image">???? ?? ?????</label>-->
                                                    <!--                                <label for="passport_image">???? ?????</label>-->
                              ...
                              <div  v-for="(item,index) in item.footer" :key="index">
                                  ...
                                  <div v-for="(value,key,indx) in item"   v-if="key ===  SelectedLangForForm.code">
                                      ...
                                      <label for="address" style="text-align: left;" >Footer Text</label>
                                                        <div :style="footerLen !== langlength?'background-color:rgba(238, 97, 97, 0.98) !important;padding: 15px !important;border-radius: 5px !important;margin-top: 20px; !important;':'padding: 15px !important;border-radius: 5px !important;margin-top: 20px;background-color: #fff !important;'">
                                          <h3 v-html="value"></h3>
                                      </div>
    ...
    export default {
        ...
        data() {
            return {
                ...
                selectformtag:[
                    ...
                    {type:'Nominee for Travel Insurance Policy',key:'nominee',value:'',custom:0,require:1,questions:[{ en: "Nominee for Travel Insurance Policy" }, { hi: "dd" }, { ta: "????? ???????????? ??????????? ??????" },{te:"???????? ??????????? ??????? ??????"},{gu:"??????? ??????????? ?????? ???? ??????"},{kn:"?????? ???? ???????? ??????"},{mr:"?????? ???? ?????????? ????????????"}]},
                    {type:'Pre Existing Disease for Travel Insurance Policy',key:'disease',value:'',custom:0,require:1,questions:[{ en: "Pre Existing Disease for Travel Insurance Policy" }, { hi: "aa" }, { ta: "????? ???????????? ?????????? ?????? ????????? ????" },{te:"?????? ???? ????? ???? ???????? ???? ??????"},{gu:"??????? ??????????? ?????? ???? ????? ??????????? ????? ???"},{kn:"?????? ???? ?????????? ????? ???????????????? ???"},{mr:"?????? ???? ?????????? ????? ???????? ???"}]},
                ...
                languageSelect:[],
                lanCode:[],
                // multiSelectLan:[{ "code": "en", "name": "English", "nativeName": "English" },],
                selectedLang:'',
                custom_id:null,
                AllOptions:[],
                headerData:'',
       ...
       this.activeTab=0;
        ...
        created() {
            ...
            this.item.form_html.push(
                {type:'DOB',key:'dob',value:'',custom:0,require:1},
                {type:'Gender',key:'gender',value:'',custom:0,require:1,options:[{"en":[{"key":"Male"},{"key":"Female"}]},{hi:[{"key":this.hin.Male},{"key":this.hin.Female}]},{ta:[{"key":this.ta.Male},{"key":this.ta.Female}]},{te:[{"key":this.te.Male},{"key":this.te.Female}]},{gu:[{"key":this.gu.Male},{"key":this.gu.Female}]},{kn:[{"key":this.kn.Male},{"key":this.kn.Female}]},{mr:[{"key":this.mr.Male},{"key":this.mr.Female}]}]},


                {type:'Pan Card Number',key:'pan',value:'',custom:0,require:0,questions:[{ en: "Pan Card Number" }, { hi:this.hin.PanCardNo }, { ta: this.ta.PanCardNo },{te:this.te.PanCardNo},{gu:this.gu.PanCardNo},{kn:this.kn.PanCardNo},{mr:this.mr.PanCardNo}]},
                {type:'Address',key:'address',value:'',custom:0,require:1,questions:[{ en: "Address" }, { hi: this.hin.Address  }, { ta: this.ta.Address  },{te: this.te.Address },{gu: this.gu.Address },{kn: this.kn.Address },{mr: this.mr.Address }]},
                {type:'City',key:'city',value:'',custom:0,require:1,questions:[{ en: "City" }, { hi: this.hin.City }, { ta: this.ta.City },{te:this.te.City},{gu:this.gu.City},{kn:this.kn.City},{mr:this.mr.City}]},
                 // for TC/SOTC
                {type:'State',key:'state',value:'',custom:0,require:1,questions:[{ en: "State" }, { hi: this.hin.State }, { ta: this.ta.State },{te:this.te.State},{gu:this.gu.State},{kn:this.kn.State},{mr:this.mr.State}]},
                {type:'Country',key:'country',value:'',custom:0,require:1,questions:[{ en: "Country" }, { hi: this.hin.Country }, { ta: this.ta.Country },{te:this.te.Country},{gu:this.gu.Country},{kn:this.kn.Country},{mr:this.mr.Country}]},
                {type:'Pincode',key:'pincode',value:'',custom:0,require:1,questions:[{ en: "Pincode" }, { hi: this.hin.Pincode }, { ta:  this.ta.Pincode },{te: this.te.Pincode},{gu: this.gu.Pincode},{kn: this.kn.Pincode},{mr: this.mr.Pincode}]},

                {type:'Nominee for Travel Insurance Policy',key:'nominee',value:'',custom:0,require:1,questions:[{ en: "Nominee for Travel Insurance Policy" }, { hi: this.hin.NomineeforTravelInsurancePolicy}, { ta: this.ta.NomineeforTravelInsurancePolicy },{te:this.te.NomineeforTravelInsurancePolicy},{gu:this.gu.NomineeforTravelInsurancePolicy},{kn:this.kn.NomineeforTravelInsurancePolicy},{mr:this.mr.NomineeforTravelInsurancePolicy}]},
                {type:'Pre Existing Disease for Travel Insurance Policy',key:'disease',value:'',custom:0,require:1,questions:[{ en: "Pre Existing Disease for Travel Insurance Policy" }, { hi: this.hin.PreExistingDiseaseforTravelInsurancePolicy}, { ta: this.ta.PreExistingDiseaseforTravelInsurancePolicy },{te: this.te.PreExistingDiseaseforTravelInsurancePolicy },{gu:this.gu.PreExistingDiseaseforTravelInsurancePolicy},{kn:this.kn.PreExistingDiseaseforTravelInsurancePolicy},{mr:this.mr.PreExistingDiseaseforTravelInsurancePolicy}]},
                );
...
console.log(type);
            ...
            nextFooter(){
                ...
                const sublis =  this.item.languages[Count];
                ...
                if(this.numCount ===   this.item.languages.length-1){
                    ...
                    this.footerDataarray.push({[sublis.code]:content})
                    this.footerData = '';
                ...
                //     if(this.numCount ===   this.item.languages.length-1){
                //         console.log("22222");
                //         this.headerTexts.questions.push({[sublis.code]:this.headerTexts.ques[this.activeTab]})
                //         this.item.header = [];
                ...
                //         console.log(  this.item.languages.length-1)
                //     }
                //     if(this.numCount <  this.item.languages.length){
...
if(this.activeTab == 0){
    ...
    this.headerDataarray = [];
                ...
                const Count=this.numCount;
                console.log(this.item.languages[Count]);
                const sublis =  this.item.languages[Count];

                console.log("ccc",JSON.stringify(sublis.code));
                ...
                if(this.numCount ===   this.item.languages.length-1){
                    ...
                    const content =this.headerData;
                        console.log("one Sec",content)
                    this.headerDataarray.push({[sublis.code]:content})
                        this.headerData = '';
            ...
            clrLang(){
                console.log()
                this.item.languages=[{ "code": "en", "name": "English", "nativeName": "English" }];
            },

            nextTab() {
                console.log("option",JSON.stringify(this.actualfield.option))

                var question=this.actualfield.ques;
              /*  var option = this.actualfield.option;*/
                ...
                if(this.numCount ===  this.item.languages.length-1){
                    ...
                    for(let i=0 ; i<this.actualfield.option[0].length ;i++){
                        console.log("daskhdskdsd",this.actualfield.option[opLen][i]);
                    ...
                    console.log("llllllll",this.actualfield.option[0].length)


                    this.actualfield.option[this.activeTab].forEach((data)=>{
                        console.log("next button",data.key);
                        if(data.key!=""){
                    ...
                    // if(empty_count<2){
                    //     alert('Minimum 2 Filled  Required');
                    //
                    //     return  false;
                    // }
                    this.AllOptions.push(this.actualfield.option[this.activeTab]);
                    console.log("ssss",this.AllOptions)

                    this.actualfield.options.push({[sublis.code]:this.actualfield.option[this.activeTab]})

                    this.actualfield.questions.push({[sublis.code]:this.actualfield.ques[this.activeTab]})
                    ...
                    // this.actualfield.option[this.activeTab];
                }
    ...
    console.log("allOptions",this.AllOptions);
            ...
            updatemeta_banner(value, type, index) {
                uploadfile("form", type, value.target.files[0], this.item.id).then(response => {
                    ...
                    this.banner_link = window.location.origin+response.data;
