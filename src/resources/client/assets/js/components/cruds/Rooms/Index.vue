...
export default {
    data() {
        return {
            columns: [
                ...
                { title: 'Room type and size', field: 'room_type_and_size', sortable: true },
                { title: 'Bed size', field: 'bed_size', sortable: true },
                { title: 'Featured image', tdComp: DatatableFeaturedImageField, sortable: false },
                { title: 'Max count', field: 'max_count', sortable: true },
    ...
    computed: {
        ...mapGetters('RoomsIndex', ['data', 'total', 'loading', 'relationships']),
    },
    watch: {
        query: {
            handler(query) {
    ...
    methods: {
        ...mapActions('RoomsIndex', ['fetchData', 'setQuery', 'resetState']),
    }
}
</script>
